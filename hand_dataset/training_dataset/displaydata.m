...
base_dir = '/home/coenvalk/Documents/projects/conductAR/conductAR/hand_dataset/training_dataset/';
uf = dir(strcat(base_dir, 'training_data/images/'));
disp(length(uf))
for i = 15:length(uf)
    disp('hi world.')
    dot = strfind(uf(i).name,'.');
    imname = uf(i).name(1:dot-1);
    load([strcat(base_dir, 'training_data/annotations/') imname '.mat']);
    im = imread([base_dir 'training_data/images/' uf(i).name]);
    imshow(im);
